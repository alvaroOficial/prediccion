...
@st.cache
def load_data():
    df = pd.read_excel('Libro1.xlsx')  # Cambia esto por la ruta de tu archivo
...
def predict_from_date(model_fit, df, fecha_deseada):
    ...
    diferencia_meses = (fecha_deseada.year - ultimo_mes.year) * \
        12 + (fecha_deseada.month - ultimo_mes.month)
...
def main():
    ...
    fecha_deseada = st.date_input(
        "Ingrese la fecha a predecir (YYYY-MM-DD)", min_value=df.index[-1].date())
    ...
    if st.button("Predecir"):
        ...
        if error:
            ...
            st.success(
                f"La predicción para {fecha_deseada.strftime('%Y-%m-%d')} es: {prediccion:.2f}")

        # Graficar los datos reales y las predicciones extendidas
        plt.figure(figsize=(12, 6))
        plt.plot(df.index, df['Total Exportaciones'], label="Datos Reales")
        futuro_fechas = pd.date_range(start=df.index[-1] + pd.offsets.MonthBegin(), periods=(
            fecha_deseada.year - df.index[-1].year)*12 + (fecha_deseada.month - df.index[-1].month), freq='M')
        plt.plot(futuro_fechas, [prediccion] * len(futuro_fechas),
                 label="Predicción", linestyle="--", color="orange")
        plt.axvline(x=df.index[-1], color='red',
                    linestyle='--', label='Inicio de Predicción')
        plt.legend()
